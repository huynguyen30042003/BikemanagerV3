...
export default function BrandsPage() {
  ...
  const visibleBrands = useMemo(() => {
    ...
    if (statusFilter === "active") return brands.filter((brand: brandRes) => brand.isActive);
    ...
    },
    {
      ...
      render: (value) =>
        value ? (
          ...
          <span className="text-sm text-muted-foreground">Không có logo</span>
        ),
    },
    {
      key: "country",
      ...
      render: (_value, row) => (
        <div className="flex justify-center gap-2">
          ...
          <Button
            size="sm"
            variant="outline"
            onClick={() => router.push(`/admin/brands/${row.id}/edit`)}
  ...
  return (
    <div className="space-y-6 p-4 py-2">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-3xl font-bold text-foreground">Thương hiệu</h1>
          <p className="text-muted-foreground">Quản lý các thương hiệu xe</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{dataBrand?.data?.totalItems ?? 0}</div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {visibleBrands.filter((brand: brandRes) => brand.isActive).length}
            </div>
          </CardContent>
        </Card>
        ...
        <CardContent>
          {visibleBrands.length > 0 ? (
            <div className="overflow-x-auto">
              <Table<brandRes> showIndex data={visibleBrands} columns={columns} />
            </div>
          ) : (
