...
import { useEditInstallmentProvidersPageState } from "@/hooks/InstallmentProviders/PageState/useEditInstallmentProvidersPageState";
...
export default function EditInstallmentProviderPage() {
	const {
		...
		handleChange,
		handleSubmit,
	}= useEditInstallmentProvidersPageState()
	useEffect(() => {
		if (installmentProviderData) {
			setFormData((prev) => ({
				...prev,
				name: installmentProviderData?.data?.name,
				phone: installmentProviderData?.data?.phone || "",
				apiEndpoint: installmentProviderData?.data?.apiEndpoint || "",
				isActive: installmentProviderData?.data?.isActive || false,
			}));
		}
	}, [installmentProviderData, setFormData]);
	if (isLoadingInstallmentProvider) {
			...
			<div className="flex items-center gap-4">
				...
				<div>
					...
					<p className="text-muted-foreground">
						Chỉnh sửa thông tin nhà cung cấp trả góp
					</p>
				</div>
			</div>
