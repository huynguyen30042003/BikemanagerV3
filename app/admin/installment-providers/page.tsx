    ...
    <div className="p-4 md:p-8 space-y-8">
      ...
      <Card>
        ...
        <CardContent>
          <div className="">
            <div className="">
              {(installmentProviderData?.items?.length ?? 0) > 0 ? (
                <div className="overflow-x-auto">
                  <Table<InstallmentProviderResponse>
                    showIndex
                    data={installmentProviderData?.items ?? []}
                    columns={columns}
                  />
                </div>
              ) : (
            ...
            <div className="flex flex-col gap-3 pt-6 md:flex-row md:items-center md:justify-between">
              <div>
                Trang {page} / {installmentProviderData?.totalPages}
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {Array.from(
                  { length: installmentProviderData?.totalPages ?? 1 },
                  (_, index) => index + 1,
                ).map((currentPage) => (
                  <Button
                    key={currentPage}
                ...
                <Button
                  type="button"
                  variant="outline"
                  disabled={page === installmentProviderData?.totalPages}
                  onClick={() => setPage((prev) => prev + 1)}
                >
                  Next
                </Button>
