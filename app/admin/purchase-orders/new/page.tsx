  ...
  return (
    <div className="p-4 md:p-8 space-y-8">
      ...
      <div className="grid grid-cols-10 gap-8">
        <Card className="w-full max-h-[calc(100vh-156px)] grid-row-start  gap-0 col-span-6 pt-4 pb-0 space-y-4">
          <CardContent className="space-y-4 px-0">
                ...
                ProductVariantsData?.items?.map(
                  (productVariant: ProductVariantResponse) => (
                    <Card
                      ...
                      onClick={() =>
                        handleAddProductVariantList(productVariant)
                      }
                    >
                      <CardContent className="p-0 rounded-xl overflow-visible">
                        ...
                        <div className="px-4 pt-4 flex items-start justify-between gap-2">
                          ...
                          <span
                            className={`text-xs px-2 py-0.5 rounded-full font-medium whitespace-nowrap ${
                              (productVariant?.stockQuantity ?? 0) > 0
                        ...
                        <div className="p-4 pt-2 space-y-2.5">
                          ...
                          <div className="flex flex-col gap-0.5 text-xs text-gray-400">
                            <span className="text-gray-700">SKU: {productVariant?.sku}</span>
                            {productVariant?.product?.barcode && (
                              <span>
                                Mã vạch: {productVariant?.product?.barcode}
                              </span>
                            )}
                          </div>
                          ...
                          <div className="flex items-center justify-between pt-3 border-t border-gray-100 mt-2">
                            <span className="font-bold text-lg text-pink-600">
                              {productVariant?.importPrice.toLocaleString()}đ
                            </span>
                          </div>
                        </div>
                      </CardContent>
            ...
            <div className="col-span-full flex flex-col gap-3 px-4 py-4 md:flex-row md:items-center md:justify-between">
              <div>
                Trang {page} / {ProductVariantsData?.totalPages} ( Tổng sản
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {Array.from(
                  { length: ProductVariantsData?.totalPages ?? 1 },
                  (_, index) => index + 1,
                ).map((currentPage) => (
                  <Button
                    key={currentPage}
                ...
                <Button
                  type="button"
                  variant="outline"
                  disabled={page === ProductVariantsData?.totalPages}
                  onClick={() => setPage((prev) => prev + 1)}
                >
                  Next
                </Button>
        ...
        <form onSubmit={handleSubmit} className="grid-row-start space-y-6 col-span-4">
          {/* Supplier Selection */}
          <Card className="w-full flex-row gap-0">
            <div className="flex-1">
              ...
              <CardContent className="space-y-4">
                <div>
                  ...
                  <Select value={supplierId} onValueChange={handleSupplierId}>
            ...
            <div className="flex-1">
              ...
              <CardContent className="space-y-4">
                <div>
                  <label className="text-sm font-medium">
                    Nhà Kho <span className="text-red-500">*</span>
                  </label>
                  <Select value={warehouseId} onValueChange={handleWarehouseId}>
                    <SelectTrigger className="w-full">
                      <SelectValue placeholder="Nhà Kho" />
                    </SelectTrigger>
                    <SelectContent>
                      {!isLoadingWarehouse &&
                        warehouseData?.items?.map((warehouse) => (
                ...
                {warehouseId && (
                  <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg text-sm">
                    <p>
                      <span className="font-medium">Tên nhà Kho:</span>{" "}
              ...
              <CardContent>
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {listProductVariant.map((item, index) => (
          ...
          <div className="flex gap-4 pt-4">
            ...
            <Link href="/admin/purchase-orders" className="flex-1">
