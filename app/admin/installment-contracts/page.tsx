    ...
    <div className="p-4 md:p-8 space-y-8">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {formatCurrency(stats.totalLoan|| 0).split(" ")[0]}B
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="">
            <div className="">
              {(installmentContractData?.items?.length ?? 0) > 0 ? (
                <div className="overflow-x-auto">
                  <Table<InstallmentContractsRes>
                    showIndex
                    data={installmentContractData?.items ?? []}
                    columns={columns}
                  />
                </div>
              ) : (
            ...
            <div className="flex flex-col gap-3 pt-6 md:flex-row md:items-center md:justify-between">
              <div>
                Trang {page} / {installmentContractData?.totalPages}
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {Array.from(
                  { length: installmentContractData?.totalPages ?? 1 },
                  (_, index) => index + 1,
                ).map((currentPage) => (
                  <Button
                    key={currentPage}
                ...
                <Button
                  type="button"
                  variant="outline"
                  disabled={page === installmentContractData?.totalPages}
                  onClick={() => setPage((prev) => prev + 1)}
                >
                  Next
                </Button>
