"use client";

import {
  Card,
  CardContent,
  CardHeader,
  CardTitle,
} from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import Link from "next/link";
import { Trash2, Edit2, Plus } from "lucide-react";
import { useSupplierPageState } from "@/hooks/Supplier/PageState/useSupplierPageState";
import { SupplierResponse } from "@/types/supplier/suppliers";
import { Column, Table } from "@/components/ui/Table";
import ConfirmDialog from "@/components/Common/ConfirmDialog";

export default function SuppliersPage() {
  const {
    page,
    setPage,
    searchInput,
    handleSearch,
    supplierData,
    handleConfirmDelete,
    isOpenDelete,
    setIsOpenDelete,
    deleteSupplier,
    handleDeleteform
  } = useSupplierPageState();

  const columns: Column<SupplierResponse>[] = [
    {
      key: "code",
      title: "Code",
      classNameHeader: "text-left",
    },
    {
      key: "name",
      title: "Nhà phân phối",
      classNameHeader: "text-left",
    },
    {
      key: "phone",
      title: "SĐT",
      classNameHeader: "text-left",
      classNameItem: "text-left",
    },
    {
      key: "email",
      title: "Email",
      classNameHeader: "text-left",
    },
    {
      key: "taxCode",
      title: "Mã số Thuế",
      classNameHeader: "text-left",
    },
    {
      key: "address",
      title: "Địa chỉ",
      classNameHeader: "text-left",
    },
    {
      key: "contactPerson",
      title: "Người liên hệ",
      classNameHeader: "text-left",
    },
    {
      key: "actions",
      title: "Hành động",
      classNameHeader: "text-center",
      classNameItem: "text-center",
      render: (_value, row) => (
        <div className="flex justify-end gap-2">
          <Link href={`/admin/suppliers/${row.id}/edit`}>
            <Button variant="outline" size="sm">
              <Edit2 size={16} />
            </Button>
          </Link>
          <Button
            variant="outline"
            size="sm"
            onClick={() => handleDeleteform(row)}
          >
            <Trash2 size={16} />
          </Button>
        </div>
      ),
    },
  ];

  return (
    <div className="p-4 md:p-8 space-y-8">
      {/* Header */}
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-foreground">Nhà cung cấp</h1>
          <p className="text-muted-foreground">
            Quản lý danh sách nhà cung cấp
          </p>
        </div>
        <Link href="/admin/suppliers/new">
          <Button>
            <Plus size={18} />
            <span>Thêm nhà cung cấp</span>
          </Button>
        </Link>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">
              Tổng nhà cung cấp
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{supplierData?.totalItems}</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">
              Đang hoạt động
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">
              {supplierData?.items?.filter((s) => s.isActive).length}
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">
              Ngừng hoạt động
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">
              {supplierData?.items?.filter((s) => !s.isActive).length}
            </div>
          </CardContent>
        </Card>
      </div>

      {/* Search and Filter */}
      <Card>
        <CardHeader>
          <CardTitle>Tìm kiếm và lọc</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <Input
            placeholder="Tìm theo tên, email hoặc số điện thoại..."
            value={searchInput}
            onChange={(e) => handleSearch(e.target.value)}
            className="w-full"
          />
        </CardContent>
      </Card>
      <Card>
        <CardHeader>
          <CardTitle>Danh sách nhà cung cấp</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="">
            <div className="">
              {(supplierData?.items?.length ?? 0) > 0 ? (
                <div className="overflow-x-auto">
                  <Table<SupplierResponse>
                    showIndex
                    data={supplierData?.items ?? []}
                    columns={columns}
                  />
                </div>
              ) : (
                <p className="text-center text-muted-foreground py-8">
                  Không tìm thấy nhà cung cấp nào
                </p>
              )}
            </div>
            <div className="flex flex-col gap-3 pt-6 md:flex-row md:items-center md:justify-between">
              <div>
                Trang {page} / {supplierData?.totalPages}
              </div>

              <div className="flex flex-wrap gap-2">
                <Button
                  type="button"
                  variant="outline"
                  disabled={page === 1}
                  onClick={() => setPage((prev) => prev - 1)}
                >
                  Previous
                </Button>

                {Array.from(
                  { length: supplierData?.totalPages ?? 1 },
                  (_, index) => index + 1,
                ).map((currentPage) => (
                  <Button
                    key={currentPage}
                    type="button"
                    variant={page === currentPage ? "default" : "outline"}
                    onClick={() => setPage(currentPage)}
                  >
                    {currentPage}
                  </Button>
                ))}

                <Button
                  type="button"
                  variant="outline"
                  disabled={page === supplierData?.totalPages}
                  onClick={() => setPage((prev) => prev + 1)}
                >
                  Next
                </Button>
              </div>
            </div>
          </div>
        </CardContent>
      </Card>
      <ConfirmDialog
        open={isOpenDelete}
        onOpenChange={setIsOpenDelete}
        variant="delete"
        title="Xóa purchase-order"
        description={`Bạn có chắc muốn xóa nhà phân phối này không?`}
        confirmText="Xóa"
        loading={deleteSupplier.isPending}
        onConfirm={handleConfirmDelete}
      />
    </div>
  );
}
